    ...
    for epoch in range(epochs):
        ...
        for i in tqdm(range(0, train_length, batch_size * downsampling)):
            ...
            with tf.GradientTape() as tape:
                ...
                loss = model.loss.call(batch_labels, mu)#model.loss.call(batch_labels, mu, sigma)
        ...
        if not len(test_data) == 0:
            if verbose >= 1:
                ...
                test_loss = model.loss.call(batch_labels, mu)#model.loss.call(test_batch_labels, test_mu, test_sigma).numpy()
...
def main(save=True, load=False, train=True, load_model=False, save_model=True):
    ...
    feature_model = models.FeatureDistributionModel(hidden_size=128, num_layers=4)
    train_distribution_model(feature_model.feature_distribution_nn, train_features, train_feature_labels, test_features, test_feature_labels, epochs=50, batch_size=128, downsampling=32, verbose=2, summary=True)
    ...
    print_results([mean_model, guess_model, randomized_guess_model, feature_model], 
