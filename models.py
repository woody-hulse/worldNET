...
class FeatureDistributionNN(tf.keras.Model):
    ...
    def __init__(self, hidden_size=8, num_layers=4, name="feature_distribution_nn"):
        ...
        self.mu_layer = tf.keras.layers.Dense(2, activation="linear")
        self.sigma_layer = tf.keras.layers.Dense(1, activation="linear")

        self.loss = tf.keras.losses.MeanSquaredError(reduction = tf.keras.losses.Reduction.SUM)# MeanNormalHaversineDistanceLoss()
...
class FeatureDistributionModel():
    ...
    def __init__(self, hidden_size=8, num_layers=2, name="feature_distribution_model"):
        ...
        self.loss = tf.keras.losses.MeanSquaredError()# MeanNormalHaversineDistanceLoss()
        self.optimizer = tf.keras.optimizers.Adam(0.01)


    def call(self, x):
        mean_preds, sigma_preds = self.feature_distribution_nn(x)
